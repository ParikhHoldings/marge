...
- AI calls: Anthropic preferred in `app/services/marge.py` when `ANTHROPIC_API_KEY` is set; OpenAI fallback when `OPENAI_API_KEY` is set. Assistant chat in `app/routers/assistant.py` is deterministic and context-backed, with LLM-backed generation used only where the service layer explicitly calls it.
...
- `docs/CODEX_HANDOFF.md`: concise current handoff for another Codex model, including objective, completed changes, files changed, blockers, next steps, tests, and avoid list.
- `docs/PILOT_CONNECTOR_RUNBOOK.md`: operator runbook for connecting the first real pilot provider, proving no-sync credential verification, and avoiding MCP/OAuth-callback/setup-only false readiness.
...
- `app/services/secure_tokens.py`: Fernet encryption helpers for OAuth token and workspace API-key credential payloads. Requires `MARGE_ENCRYPTION_KEY`.
...
- `app/routers/chat.py`: legacy compatibility endpoint that delegates to account-scoped `/assistant/chat`.
...
Common local settings are documented in `.env.example`. That template also documents the local connector boundary: env values alone do not prove a pastor's live tools are connected, MCP/local API access is not a live church-tool provider, and Check credentials must run before the first sync.

- `DATABASE_URL`: defaults to `sqlite:///./marge.db`.
- `PASTOR_NAME`: legacy local fallback only when no workspace profile is selected.
- `CHURCH_NAME`: legacy local fallback only when no workspace profile is selected.
...
- `ROCK_API_KEY` / `ROCK_BASE_URL`: optional server-wide Rock RMS key and public HTTPS API v2 base URL with no username, password, query string, fragment, localhost, or private-network host. Prefer encrypted workspace credentials for multi-church use.
- `MARGE_ENCRYPTION_KEY`: Fernet key required before OAuth connector tokens or workspace API-key connector credentials can be stored.
...
- `BREEZE_API_KEY` / `BREEZE_BASE_URL`: optional server-wide Breeze API key and public HTTPS account URL with no username, password, query string, fragment, localhost, or private-network host. Prefer encrypted workspace credentials for multi-church use.
...
- MCP and live verification: `MARGE_API_URL` plus `MARGE_ACCOUNT_TOKEN` for scoped workspace access. The token is required for `--require-live-provider` checks, even against localhost, and for any non-local live connector verification.

Never commit real `.env`, API keys, local DB files, or pastoral/member data exports.
The default `artifacts/` directory is ignored because live verification evidence can include provider identity metadata and workspace counts. Do not commit generated pilot evidence reports.
...
- `POST /assistant/integrations/{provider}/credentials` stores encrypted workspace API-key credentials for API-key connectors such as Breeze and Rock. Owner/admin only; require a full public HTTPS base URL with no username, password, query string, fragment, localhost, or private-network host; never return the submitted key in status, chat, audit, or frontend logs. Frontend setup cards should say `Add encrypted credentials` only when `MARGE_ENCRYPTION_KEY` is available, and `Review encrypted storage` when encrypted credential storage itself is missing.
...
- `scripts/smoke_pilot_readiness_gate.py` is the no-network coverage gate for the combined pilot readiness planner. It should prove pilot readiness refuses first-run writes and live connector checks until `MARGE_ACCOUNT_TOKEN` is present; non-local tokens must resolve to owner/admin/pastor access before the gate continues.
...
- Synced inbox reply drafts should match the sender to local Marge people memory by email or full display name when possible. If the matched member has active care/prayer context or `context_tag="preference"` notes, carry that local memory in `payload.draft_context.member_context` and `payload.draft_context.member_preferences` for pastor review while keeping `payload.email.body` free of sensitive metadata by default.
...
- `POST /members/sync/rock` is legacy compatibility only. Keep it delegated to the assistant Rock connector sync path so it cannot bypass encrypted workspace credentials or the verify-before-sync boundary.
...
- Synced Google Workspace/Microsoft 365 meeting-prep actions should match organizers/attendees to local Marge people memory by email or full display name when possible. If matched members have active care/prayer context or `context_tag="preference"` notes, carry that local memory in `payload.review_context.member_context` and `payload.review_context.member_preferences` for pastor review; this is local prep context, not an external calendar write.
...
- Remembered person preferences such as "Remember that Janet Ellis prefers phone calls over texts" should save a local `MemberNote` with `context_tag="preference"` when the person is matched. This is local Marge memory, not profile context and not an external write. Later person-context lookups should surface these notes distinctly as preferences to respect, not bury them as generic recent notes. Follow-up drafts for matched members should carry these preferences in `payload.draft_context.member_preferences` as pastor-only review metadata, not paste them into `payload.email.body` by default.
...
- Ministry recap prompts such as "What have you learned about my ministry?" should explicitly summarize the saved role/church, local context, church voice/tradition, follow-up pressure, first priority, personal support style, tools, drafting voice, weekly rhythm, and approval boundaries. Do not make the pastor infer rhythm or guardrails only from setup cards.
- Learning-gap prompts such as "What else do you need from me?" or "What do you still need to learn?" should distinguish the completed core ministry profile from the next missing real inputs: first visitor/care/prayer/member context, member preferences, synced inbox/calendar/ChMS context, and credential-checked tools. Attach the concrete `data_seed` or connector setup/check cards instead of repeating a generic operating-plan recap.
- Profile-setting lookup prompts such as "What are my guardrails?", "How should you sound when drafting?", "What rhythm should you protect?", and "What tools do you remember we use?" should answer from saved `profile.guardrails`, `profile.communication_style`, `profile.faith_tradition`, `profile.weekly_rhythm`, and `profile.tools_in_use`. Rhythm lookups must preserve calendar write/approval boundaries, and tool lookups must keep the setup -> no-sync credential check -> pastor-requested sync order visible with connector setup/check cards when available. Do not answer these with generic approval rules, drafting advice, scheduling advice, or connector copy when saved pastor context exists.
- Pre-meeting context questions such as "What should I know before meeting with Janet Ellis?" should route to person context and combine local people memory with synced calendar context when a Google Workspace/Microsoft 365 event matches the member. Return the synced calendar card; do not fall through to generic scheduling or meeting lookup copy.
- Pastoral history questions such as "When did I last visit Janet Ellis?" or "Have I followed up with Janet Ellis?" should route to person context and answer from local care/prayer/note memory before generic calendar planning. These are lookup questions, not new contact logs.
- Natural triage questions such as "Who should I check on next?" should route to next-action triage and answer from current care, visitor, prayer, absence, setup, and approval context rather than generic advice. When the top item is a named care case, suggested prompts should carry that name forward into a care draft and local reminder.
...
- First-run ministry profile saves should reflect back the actual context Marge learned and explain how she will use it. Avoid generic "saved" replies when the pastor gives unique role, church, follow-up, personal support, tool, voice, or guardrail context.
...
- First-run completion should require the pastor's size, church voice/tradition, first ministry priority, personal support style, rhythm, and guardrail context, not just a default approval sentence. Marge should not declare the profile complete or prepare launch/setup actions until she has learned what language to respect, what the pastor wants moved first, how he wants to be nudged and protected, the weekly ministry rhythm, and explicit boundaries.
...
- Suggested first-run prompts must be real chat behaviors too. "How will you use this context?" should explain how saved ministry context shapes priorities, support style, drafts, secure setup, and approvals. "How will you support me?" should answer from `profile.support_preferences` when present and ask the next context question when it is missing. "Why is this the next step?" should explain the active setup step in terms of the pastor's saved follow-up burden, tools, support preferences, or missing context and attach the same setup card. "What should I do for {saved ministry priority}?" should answer from `profile.ministry_priorities` plus active setup/priority records, not generic fallback. "Show my setup steps" should return concrete setup cards, and coaching prompts such as "What should I ask a new family?", "What should I record first?", or "How do you handle private prayer?" should explain what to capture and keep the relevant `data_seed` card attached.
...
- Secondary-view `Tell Marge` panels should render the same enriched chat body as the Assistant conversation. Do not reduce action-bearing replies to text-only bubbles on People, Care, Visitors, Prayer, Email, Calendar, Integrations, Agent Tools, or Plan.
...
- Natural setup phrasing such as "connect my email", "set up my mail", "connect my calendar", or "set up my schedule" should route to secure connector setup and let saved tools choose the provider. Do not require the pastor to say "Google Workspace" or "Microsoft 365" before Marge can begin the right setup flow.
...
- Pastoral pressure prompts such as "I'm overwhelmed", "I'm exhausted", or "ministry feels heavy" should route through saved support preferences and weekly rhythm, then attach the smallest concrete next setup/action item. Do not answer these as generic encouragement or create work; keep sends, calendar writes, connector syncs, and system changes behind approval.
- Chat prompts like "Prepare my first-week plan" should return the reviewable `first_week_plan` action and summarize the pastor's first real record, recommended connectors, personal support style, rhythm, voice, and approval guardrails. Do not let a prompt Marge suggests fall through to the generic assistant reply.
- Off-script chat in a complete but still-empty first-run workspace should still attach the active setup step and setup-aware prompt chips. Do not let unmatched wording like "Can you be my secretary?" answer as if the desk is clear before the first real ministry record or connector check exists. Once real people/context exist, unmatched chat should lead with those real priorities before connector setup.
...
- First-run profile extraction should handle natural pastor wording, not only form-like answers: "I'm a bi-vocational solo pastor," "we have 85 on Sundays," "our church tradition is non-denominational with Baptist roots," "my first priority this month is closing loops with first-time guests," "nudge me gently," "our stack is Planning Center and Gmail," and "ask me before sending" should save role, size, church voice, ministry priorities, support preferences, tools, and guardrails without contaminating weekly rhythm or logging generic guest language as a specific visitor.
...
- First visitor welcome drafts should use the actual visitor note when present, such as "asked about kids ministry", and should carry saved drafting voice, church voice/tradition, personal support style, and approval guardrails in review metadata. Do not leak that metadata into provider MIME/send payloads.
...
- Chat reminder requests such as "Remind me to call Janet Ellis tomorrow" should queue a local `pastoral_reminder` assistant action, link a known local person when possible, preserve timing language, and explicitly say nothing was sent, synced, or written externally. Do not turn reminder phrasing into a fake person or an external calendar write.
- Reminder lookup prompts such as "What reminders do I have?" or "Show my reminders" should list pending local `pastoral_reminder` actions with reminder cards and say they are local Marge memory only. Do not answer these as generic approval-queue copy.
- After chat logs a pastoral contact, it should proactively suggest a local next-check-in reminder such as `Remind me to check on Janet Ellis next week.` Keep this as a suggested local action, not an automatic external calendar write.
- Frontend `pastoral_reminder` actions should show a local `Mark done` path, not generic external approval language. The button may execute the local item directly, but it must still say this is local Marge memory and will not send, sync, or write externally. Natural chat commands such as "Mark Janet reminder done" should complete the matching local reminder without requiring a separate approval, "Cancel Janet reminder" should skip the matching local reminder, and "Move Janet reminder to Friday" or "Snooze it in two weeks" should update only the local reminder timing without touching external systems.
- Prayer and care follow-up drafts should use the actual request/care situation in the body when safe, and should carry saved drafting voice, church voice/tradition, personal support style, and privacy/approval guardrails in review metadata. Do not reduce a grief, diagnosis, job-loss, or hospital note to generic pastoral copy.
- Frontend person detail should surface `context_tag="preference"` notes under `Preferences To Respect`, separate from generic recent notes, so a pastor can see that Marge remembered relational preferences.
...
- Live chat help and suggested prompts should not teach with fake people or `.test` email addresses. When Marge needs an example format, derive it from real workspace care/visitor/prayer context when available; otherwise use neutral wording such as "the follow-up you want protected."
- Missing names from care, visitor, absence, action-modal, or synced provider rows should stay visibly incomplete, not invented. Use `Name not linked`, `Visitor name not linked`, or `Member name not linked` for display-only context, preserve partial names when a provider sends them, and skip unusable no-name provider rows instead of creating people named `Unknown`.
...
- Briefings, assistant greetings, and message drafts should avoid doubled titles. Use `pastor_display_name(...)` from `app/services/marge.py` before inserting a pastor name into greeting or draft copy; do not format `"Pastor {name}"` around an already titled name.
...
- Synced inbox rows without queued actions should offer a chat prompt to draft a reviewable reply from that real synced email, not send pastors to a generic approval queue.
- Calendar UI should reflect actual connector state and synced `ConnectedContextItem` calendar rows. Synced meetings should open their meeting-prep/review actions when present; when no action exists yet, offer a chat prompt to prepare a brief from that real synced calendar item instead of remaining hidden behind generic calendar guidance. Live scheduling proposals should queue or open real `calendar_block` assistant actions; do not use browser-only scheduling draft previews for approval-looking work.
...
- Frontend verification modals should defensively hide token/API-key/session-shaped identity metadata and redact token-shaped free-text messages even though the backend should already reject them. Do not render raw provider identity fields or messages with names or values that look like tokens, secrets, cookies, passwords, or bearer/JWT values.
- Frontend execution summaries for provider writebacks should redact provider IDs and suppress external links that are not HTTPS or that contain token-shaped query/text. Provider-created IDs and links are useful for review, but they must never become a secret display path.
- Backend chat replies that include connector verification, sync, or action-execution failure details must pass provider/HTTP text through `_redact_secret_text()` before Marge says it back to the pastor. Chat is not a diagnostic log and must not echo OAuth/API/session values even when an upstream provider includes them in an error.
...
- Connector setup, API-key credential storage, credential verification, connector sync, writeback policy changes, disconnect, and legacy Rock sync all require a real Marge workspace even when local legacy token mode is relaxed. Do not allow OAuth state rows, API-key credentials, synced context, or connector policy writes to land in unscoped legacy rows.
...
- MCP is a local agent bridge, not a church-tool provider. `list_integrations` should make clear that MCP access lets LLM clients call Marge but does not prove Google Workspace, Planning Center, Microsoft 365, Breeze, or Rock RMS are connected; live readiness still requires a verified external connector.
- Backend MCP verification may report local bridge availability for diagnostics, but it must not use `credentials verified` wording, must return `verified_at=null`, must not persist `verified_at` as if MCP were an external connector, and must not count toward live provider readiness.
- MCP list outputs should use the same pastoral missing-context language as the web app. For unlinked care cases use `Name not linked`; for incomplete member rows use `Member name not linked`; for private or unnamed prayer requests use `Name withheld`. Preserve partial names when providers send them. Do not surface `Unknown`, `Anonymous`, or raw Python `None` placeholders to external LLM clients.
- MCP API failures should surface the backend's safe `detail` message through `_http_error_detail`, not raw JSON response bodies, stack traces, or transport noise. MCP error/detail/message rendering should also redact token/API-key/session-shaped values before returning text to external LLM clients, including connector messages, identity summaries, compact execution payloads, and generic exception text.
- MCP rendered tool output should also pass through `_redact_secret_text()` anywhere it formats saved chat history, assistant desk fields, ministry profile fields, connected-context titles/snippets/provider labels, prayer/care/member names, draft text, connector setup/sync/verify/disconnect messages, returned action cards, and suggested prompts. External LLM clients can replay whatever MCP returns, so do not expose pastor-pasted tokens even when they were saved into normal text fields.
- MCP connected-context and approval-list outputs should describe incomplete provider/action fields directly, such as `title not included by provider`, `date not included by provider`, or `Assistant action title not included`. Do not use placeholder labels like `Untitled` or `unknown date`.
- MCP empty states should be actionable and honest. Empty members, care, prayer, connected-context, or approval-list tools should point to the first real ministry record, secure connector setup/check/sync order, or reviewable work; do not answer with dead-end `No X found` copy that implies the workspace has been fully checked.
- MCP prayer creation should preserve a provided unlinked name as `submitted_by` when no saved member matches, and tell the agent it is not linked to a saved member yet. Do not silently turn a named prayer request into an unnamed/private request.
- MCP write tools that require a saved member, such as care events, member notes, or drafts, should refuse unmatched names without mutating data and point the agent to safe person capture through `tell_marge`. Do not dead-end on `Try list_members` when the agent already supplied a real name.
- MCP member lookup for write tools should require an exact full-name match or a single search result. Do not silently pick the first result from an ambiguous partial-name search.
...
- Chat setup-card helpers should compute integration status with the current workspace user whenever a user token exists. Even data-seed or missing-name guidance can attach setup cards, so do not call `_integration_statuses(db, account)` from those paths when `access.user` is available.
- `POST /assistant/integrations/{provider}/verify` should make only a minimal provider identity/config call, audit `integration.verified`, and never import ministry data, queue actions, or return secret-shaped identity metadata such as token/API-key fields or token-shaped values. Identity metadata should use real credential-health labels such as `people_access_confirmed`, not sample/demo wording. Empty, false-only, zero-only, or negative numeric identity/config metadata must fail instead of setting `verified_at`. Use this endpoint to prove credentials before sync.
- `scripts/verify_live_integrations.py` calls the live verify endpoint against `MARGE_API_URL` using `MARGE_ACCOUNT_TOKEN`. The token is required for `--require-live-provider` even on localhost, and for any non-local API check, so verification is scoped to the pilot workspace before provider credentials are checked. Strict live-provider runs must resolve the token to owner/admin/pastor access before provider checks. It is the operator-facing credential health check and should remain read-only/no-sync. It snapshots every page of connected context, assistant actions, members, visitors, care cases, and prayer requests, including private prayer requests, before/after verification so credential checks cannot quietly import data, mutate records, or queue work outside the first page. It refuses to count providers whose verify response exposes secret-shaped identity fields or lacks affirmative non-secret identity/config metadata. Production/pilot checks should include `--require-live-provider`; MCP alone is not proof that a pastor's live tools are connected. Human output should separate `External provider credential checks` from `Local agent bridge checks (not live providers)`, and JSON evidence should keep `external_provider_checks` separate from `local_bridge_checks` plus `external_verified_count` separate from `local_bridge_checked_count`. The JSON evidence should also include `generated_at`, token-free `workspace` scope, `live_provider_ready`, and `no_sync_side_effect_check_passed`; a provider is not ready if the no-sync side-effect check is skipped, missing, or failed. The legacy `verified` array remains for compatibility only; do not use it as readiness proof. If no external provider qualifies, keep its human `Next actions` and JSON `next_actions` telling operators to connect/configure a real provider, run Check credentials, rerun with `--include-mcp --require-live-provider`, and avoid first sync until it passes.
- `scripts/verify_production_config.py` should pass before exposing real pastoral data. It accepts `--env-file` for checking a candidate production env file without replacing local `.env`; when this flag is used, checked production keys must come from the candidate file and should not be silently filled by ambient shell variables. It does not contact providers; it catches unsafe deployment settings that would undermine secure connectors or workspace isolation, including non-public Breeze/Rock server-side base URLs or base URLs with embedded credentials/query strings. If it fails, preserve the human `Next actions` and JSON `next_actions` so operators know which production settings to fill next.
...
- `scripts/verify_live_integrations.py --evidence-file artifacts/live-connector-verification.json` writes a structured token-free evidence report for pilot handoff. Keep it free of workspace tokens, OAuth tokens, API keys, provider payload dumps, and private pastoral text. The verifier redacts obvious token-shaped values in messages/errors before returning or writing evidence; route any new evidence fields through that sanitizer too.
- `scripts/verify_pilot_readiness.py` is the one-command pilot gate. It runs production config, public deployment bootstrap, migration drift, a disposable first-run workspace rehearsal, no-sync live connector verification, and validation of the generated live evidence JSON. It should fail unless at least one supported external provider verifies successfully, the evidence `api_url` matches the current `MARGE_API_URL`, the evidence says `required_live_provider=true`, the evidence has a valid timezone-bearing `generated_at` timestamp from the last 24 hours and not future-dated beyond clock skew, the evidence includes token-free workspace scope with owner/admin/pastor role, the evidence says `live_provider_ready=true` plus `no_sync_side_effect_check_passed=true`, and `side_effect_check.collections` includes passing checks for connected context, assistant actions, members, visitors, care cases, and prayer requests. It must reject secret-shaped keys and obvious secret-shaped values anywhere in the evidence report. Supported live providers are Google Workspace, Microsoft 365, Planning Center, Breeze, and Rock RMS. It requires `MARGE_ACCOUNT_TOKEN` before first-run writes or live connector checks; for non-local API URLs it preflights that token against `/assistant/account` and accepts only owner/admin/pastor access. It clears any previous live evidence file before rerunning the live verifier, then passes `--evidence-file artifacts/live-connector-verification.json` by default; use `--live-evidence-file <path>` for another handoff artifact path. Use `--skip-first-run-workspace` only for targeted troubleshooting. Summary next actions should stay copy-pasteable, including exact `.venv/bin/python scripts/verify_production_config.py` and `MARGE_API_URL=... MARGE_ACCOUNT_TOKEN=REPLACE_WITH_OWNER_ADMIN_PASTOR_SESSION .venv/bin/python scripts/verify_live_integrations.py --include-mcp --require-live-provider --evidence-file artifacts/live-connector-verification.json` rerun commands.
- Candidate env files may include blank operator fields like `MARGE_ACCOUNT_TOKEN=`. The pilot gate should preserve non-empty shell-provided `MARGE_ACCOUNT_TOKEN` and `MARGE_API_URL` values when the env file value is blank; explicit `--token` and `--api-url` flags still override both.
- `.env.production.example`, `docs/PRODUCTION_READINESS.md`, and `docs/PILOT_CONNECTOR_RUNBOOK.md` are the production handoff artifacts. Keep them aligned with connector requirements, OAuth redirect paths, migration expectations, and the live no-sync credential verification flow.
...
- `GET /` is also public. Keep it as a generic app pointer only; do not return `PASTOR_NAME`, `CHURCH_NAME`, connector state, or workspace data before a session exists.
...
- Before a workspace exists, any live chat composer should answer locally with workspace/setup-safe guidance instead of calling `/assistant/chat`. Scoped assistant chat requires a real workspace session; do not make non-Assistant tabs leak no-workspace chat into backend errors. These local replies should still include a visible `Create workspace` card and setup prompt chips so the chat never dead-ends as plain text.
- First-pastor signup should require a church name and valid owner email so the workspace is scoped and recoverable. The visible form should also require pastor name, while backend/API smoke paths may still exercise missing pastor-name profile fields so Marge can ask and save them through chat.
- First-pastor signup placeholders should feel like real church account setup, not generic sample data. Prefer `you@yourchurch.org` over `pastor@example.com` in visible forms.
